...
// GetIPFromMetadata extracts the client IP from gRPC metadata.
// Priority:
// 1. x-forwarded-for
// 2. x-real-ip
// Returns empty string if no valid IP found.
func GetIPFromMetadata(md metadata.MD) string {
	...
	// 2) X-Real-IP
	if xr := md.Get("x-real-ip"); len(xr) > 0 {

		if parsed := parseIP(xr[0]); parsed != "" {
			return parsed
		}
	}
...
// parseIP validates and returns canonical IP string
func parseIP(s string) string {
